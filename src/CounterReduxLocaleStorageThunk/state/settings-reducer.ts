...
// thunk
export const setStartValueTC = (): AppThunkType => dispatch => {
    const count = localStorage.getItem('valueStart');
    if (count) {
        dispatch(changeStartValueAC(JSON.parse(count)));
    }
}
export const setMaxValueTC = (): AppThunkType => dispatch => {
    const count = localStorage.getItem('valueMax');
    if (count) {
        dispatch(changeMaxValueAC(JSON.parse(count)));
    }
}